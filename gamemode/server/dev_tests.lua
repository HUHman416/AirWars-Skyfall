...
-- Server-side only. All commands that mutate game state require admin/server console access.

local function test_authorized(ply)
	return !IsValid(ply) or ply:IsAdmin()
end
...
local function get_test_ship(ply, args)
	if !istable(world_ships) then return nil end
...
local function remove_team_build_props(team_id)
	...
	for _, ent in ipairs(ents.FindByClass("aw_building_prop")) do
		if !IsValid(ent) or !ent.GetAWTeam then continue end
		if ent:GetAWTeam() != team_id then continue end
		ent:Remove()
		removed = removed + 1
	end
	return removed
...
function AirWars:RunSkyfallSmokeTest(ply)
	...
	if team_count == 0 then
		...
		for id, team in pairs(aw_teams_list) do
			...
			check("team " .. tostring(id) .. " members", istable(team.members))
	...
	for _, ply_test in ipairs(player.GetAll()) do
		...
		elseif !istable(aw_teams_list) or !aw_teams_list[team_id] then
	...
	if istable(game_state) and game_state.state == GAME_STATE_FIGHT then
		...
		if controllers > 0 then result("PASS", "fight controllers", controllers) else result("WARN", "fight controllers", "none found") end
		if spawns > 0 then result("PASS", "fight player spawns", spawns) else result("WARN", "fight player spawns", "none found") end
	end
...
concommand.Add("aw_test_help", function(ply)
	if !test_authorized(ply) then return end
...
concommand.Add("aw_test_buildship", function(ply)
	if !test_authorized(ply) or !IsValid(ply) then return end
	if !istable(game_state) then
	...
	-- A standardized v0.1 test craft: hull, propulsion/lift, helm, spawn, ammo, and every legacy ship weapon.
	add(Vector(0, 0, 0), CATEGORY_PROPS, 1)
	add(Vector(0, 0, 48), CATEGORY_SPECIALS, 1)
	add(Vector(-110, 0, 70), CATEGORY_SPECIALS, 2)
	add(Vector(0, 0, 190), CATEGORY_SPECIALS, 4)
	add(Vector(95, 0, 48), CATEGORY_SPECIALS, 5)
	add(Vector(0, 90, 48), CATEGORY_SPECIALS, 6)
	add(Vector(0, -100, 55), CATEGORY_WEAPONS, 1)
	add(Vector(70, -100, 55), CATEGORY_WEAPONS, 2)
	add(Vector(-70, -100, 55), CATEGORY_WEAPONS, 3)
	add(Vector(0, 135, 55), CATEGORY_WEAPONS, 4)

	test_reply(ply, "Test ship created: " .. spawned .. "/10 parts spawned; removed " .. removed .. " previous build parts")
	if spawned < 10 then
		test_reply(ply, "[WARN] Some parts failed to spawn. Run aw_test_all and check funds/entity errors.")
	else
		test_reply(ply, "Next: aw_test_startfight")
	end
end)
...
concommand.Add("aw_test_startfight", function(ply)
	if !test_authorized(ply) then return end
	if !isfunction(AirWars.StartRound) then
		test_reply(ply, "StartRound is unavailable")
		return
	end

	if !istable(aw_teams_list) or table.Count(aw_teams_list) <= 1 then
		aw_developer = true
...
concommand.Add("aw_test_reset", function(ply)
	if !test_authorized(ply) then return end
	if !isfunction(AirWars.ResetRound) then
...
concommand.Add("aw_test_roundtrip", function(ply)
	if !test_authorized(ply) then return end
	if !istable(game_state) or game_state.state == GAME_STATE_FIGHT then
